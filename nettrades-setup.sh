...
${YELLOW}OPTIONS (CLI):${NC}
    ...
    --with-kai            Deploy KAI Scheduler for GPU scheduling (Kubernetes only).
    --platform            Override platform detection (linux, macos, wsl).
...
run_interactive() {
    ...
    echo ""
    echo "Optional components (for future scaling):"
    echo "  Grove observability platform (unifies Prometheus, Loki, Tempo)"
    read -rp "Deploy Grove? (y/N): " grove_yn
    [[ "$grove_yn" =~ ^[Yy]$ ]] && WITH_GROVE=true || WITH_GROVE=false

    echo "  KAI Scheduler for GPU-aware Kubernetes scheduling"
    read -rp "Deploy KAI Scheduler? (y/N): " kai_yn
...
log_info "With Grove: $WITH_GROVE"
log_info "With KAI Scheduler: $WITH_KAI"
