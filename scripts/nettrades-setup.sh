...
setup_dev_environment() {
    ...
    if [[ -f "$PROJECT_ROOT/requirements-dev.txt" ]]; then
        ...
        if ! output=$(pip3 install --break-system-packages --verbose -r "$PROJECT_ROOT/requirements-dev.txt" 2>&1); then
            log_error "Python dependency installation failed:"
            echo "$output" >&2
            log_info "Try running: pip3 install --break-system-packages --verbose -r requirements-dev.txt"
            exit 1
        fi
    else
