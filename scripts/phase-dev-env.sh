...
install_dependencies() {
    ...
    pip install --upgrade pip setuptools wheel --break-system-packages --prefer-binary --verbose
    ...
    for pkg in torch transformers datasets accelerate; do
        if pip show "$pkg" &> /dev/null; then
            ...
            pip install "$pkg" --break-system-packages --prefer-binary --verbose
        fi
    done
    ...
    if [ -f "$ODOO_LLM_REQS" ]; then
        ...
        pip install --break-system-packages --prefer-binary --verbose -r "$ODOO_LLM_REQS"
    ...
    if [ -f "$ODOO_REQS" ]; then
        ...
        pip install --break-system-packages --prefer-binary --verbose -r "$ODOO_REQS"
    ...
    if pip show prometheus-client &> /dev/null; then
        ...
        pip install prometheus-client --break-system-packages --prefer-binary --verbose
    fi
    ...
    pip install --upgrade langgraph-cli --break-system-packages --prefer-binary --verbose
    ...
    pip install --upgrade "starlette>=1.0.1" --break-system-packages --prefer-binary --verbose
