...
# =============================================================================
# This script is safe to re-run. It checks for existing files, images,
# and directories before creating them. It uses the shared detection
# library to decide between CPU (llama.cpp) and GPU (vLLM) inference.
...
mkdir -p addons postgres-data odoo-data forgejo-data llama-cpp-data \
         prometheus-data grafana-data traefik-data backups \
         gpustack-data valkey-data config
...
if ! docker image inspect mcp-odoo-server:latest &>/dev/null; then
    echo "Building MCP-Odoo bridge image..."
    docker build -t mcp-odoo-server:latest ../mcp-odoo
fi
...
CREATE TABLE IF NOT EXISTS nettrades_experience (id SERIAL PRIMARY KEY, partner_id INTEGER NOT NULL, job_title VARCHAR(255), company VARCHAR(255), start_date DATE, end_date DATE, description TEXT, created_at TIMESTAMP DEFAULT NOW());
CREATE TABLE IF NOT EXISTS nettrades_review (id SERIAL PRIMARY KEY, reviewer_id INTEGER NOT NULL, reviewed_partner_id INTEGER NOT NULL, rating INTEGER CHECK(rating>=1 AND rating<=5), comment TEXT, project_id INTEGER, created_at TIMESTAMP DEFAULT NOW());
CREATE TABLE IF NOT EXISTS nettrades_user_match (id SERIAL PRIMARY KEY, job_id INTEGER NOT NULL, user_id INTEGER NOT NULL, match_score FLOAT, analysis TEXT, created_at TIMESTAMP DEFAULT NOW());
CREATE TABLE IF NOT EXISTS forgejo_repo (id SERIAL PRIMARY KEY, project_id INTEGER NOT NULL, repo_url VARCHAR(512), clone_url VARCHAR(512), created_at TIMESTAMP DEFAULT NOW());
CREATE TABLE IF NOT EXISTS nettrades_field (id SERIAL PRIMARY KEY, name VARCHAR(255), description TEXT, only_qualified BOOLEAN DEFAULT FALSE, reputation_threshold_for_charging INTEGER DEFAULT 100, base_points_per_vote INTEGER DEFAULT 1, qualified_points_per_vote INTEGER DEFAULT 5, created_at TIMESTAMP DEFAULT NOW());
